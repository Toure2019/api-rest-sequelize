		...
		asyncLib.waterfall([
			...
			function(messageFound, done) {
				if (messageFound) {
					...
					res.status(404).json({ 'error': 'post already liked (-return)' });
				}
			},
			...
			function(messageFound, userFound, done) {
				messageFound.update({
					...
					res.status(500).json({ 'error': 'cannot update message like counter ' + err });
				});
			}
		...
		asyncLib.waterfall([
			...
			function(messageFound, userFound, done) {
				messageFound.update({
					...
					res.status(500).json({ 'error': 'cannot update message like counter (-return)' });
				});
			},
		], function(messageFound) {
			if (messageFound) {
